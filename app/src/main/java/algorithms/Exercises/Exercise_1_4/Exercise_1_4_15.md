...
- **Scale the value range with $N$.** With the harness's range fixed at
  ...
  duplicates, which changes the cost being measured — `TwoSumFast`'s binary
  searches start terminating early on runs of equal keys and its doubling ratio
  sinks to 2 rather than drifting above it. The input distribution is part of
  the experiment.
